...
from DiffGesture.scripts.model.embedding_net import EmbeddingNet
from DiffGesture.scripts.model.motion_ae import MotionAE
...
class EmbeddingSpaceEvaluator:
    ...
    def push_samples(self, context_text, context_spec, generated_poses, real_poses):
        ...
        if self.pose_dim == 27:
            ...
            cos_loss_real = torch.sum(1 - torch.cosine_similarity(real_recon.view(real_recon.shape[0], real_recon.shape[1], -1, 2), real_poses.view(real_poses.shape[0], real_poses.shape[1], -1, 2), dim = -1))
        ...
        if self.pose_dim == 27:
            ...
            cos_loss_fake = torch.sum(1 - torch.cosine_similarity(generated_recon.view(generated_recon.shape[0], generated_recon.shape[1], -1, 2), generated_poses.view(generated_poses.shape[0], generated_poses.shape[1], -1, 2), dim = -1))
