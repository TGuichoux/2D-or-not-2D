...
import DiffGesture.scripts.model.embedding_net
from DiffGesture.scripts.model.tcn import TemporalConvNet
...
class PoseGenerator(nn.Module):
    ...
    def forward(self, pre_seq, in_text, in_audio, vid_indices=None):
        ...
        if self.input_context == 'both' or self.input_context=='audio':
            # audio
            audio_feat_seq = self.audio_encoder(in_audio)  # output (bs, n_frames, feat_size)  [128,34,32]

            # text
        if self.input_context == 'both' or self.input_context == 'text':
        ...
        if self.z_obj:
            if self.speaker_embedding:
                ...
                z_context = DiffGesture.scripts.model.embedding_net.reparameterize(z_mu, z_logvar)  # [128,16]
            else:
                z_mu = z_logvar = None
