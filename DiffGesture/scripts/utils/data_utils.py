...
def convert_dir_vec_to_pose_2d(vec):
    ...
    if len(vec.shape) == 2:
        ...
        joint_pos = np.zeros((vec.shape[0], 10, 2))
        for j, pair in enumerate(dir_vec_pairs):
            joint_pos[:, pair[1]] = joint_pos[:, pair[0]] + pair[2] * vec[:, j]
    elif len(vec.shape) == 4:  # (batch, seq, 9, 2)
        joint_pos = np.zeros((vec.shape[0], vec.shape[1], 10, 2))
        for j, pair in enumerate(dir_vec_pairs):
            joint_pos[:, :, pair[1]] = joint_pos[:, :, pair[0]] + pair[2] * vec[:, :, j]
    else:
        assert False
...
def convert_dir_vec_to_pose(vec,dir_vec_pairs=dir_vec_pairs):
    vec = np.array(vec)
